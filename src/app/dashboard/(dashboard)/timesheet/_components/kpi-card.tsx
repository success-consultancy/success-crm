...
const Sparkline = ({ trend }: { trend: 'up' | 'down' }) => {
  const stroke = trend === 'up' ? '#13D256' : '#E81B00';
...
const KpiCard = ({ label, value, delta, comparisonLabel, spark = 'up' }: Props) => {
  ...
  return (
    <div className="rounded-xl border border-neutral-border-light bg-white-100 p-5 flex-1 min-w-0">
      <p className="text-b14-500 text-neutral-black">{label}</p>
      <div className="mt-3 flex items-end justify-between gap-2">
        <div className="min-w-0">
          <h3 className="text-3xl font-bold text-neutral-black leading-none">{value}</h3>
          <p
            className={cn(
              'mt-2 text-b14-500 flex items-center gap-1',
              delta.positive ? 'text-utility-green' : 'text-utility-red',
            )}
          >
            <TrendIcon className="w-3.5 h-3.5" strokeWidth={2.25} />
            {delta.label}
            <span className="text-neutral-light-grey font-normal ml-1">{comparisonLabel}</span>
          </p>
        </div>
