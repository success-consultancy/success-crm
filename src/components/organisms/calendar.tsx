...
function Calendar({ className, classNames, showOutsideDays = true, validationRules,selected,onSelect, ...props }: CalendarProps) {
  const today = new Date();
  // Note: the dropdown layout is available only when fromDate, fromMonth or fromYear and toDate, toMonth or toYear are set.
  const maxDate = new Date(today.getFullYear() - 0, today.getMonth(), today.getDate()); // 0 years in the future
  const minDate = new Date(today.getFullYear() - 130, today.getMonth(), today.getDate()); // 130 years in the past
  ...
  return (
    ...
    selected={selected}
    onSelect={onSelect}
      className={cn('bg-white-700 text-content-body ', className)}
      {...props}
  />
  //   <DayPicker
  //   mode="single"
  ...
  //     startMonth={validationRules?.fromMonth || minDate}
  ...
  //       day_today: 'border border-accent-700 hover:border-accent-800  hover:text-accent-800 text-accent-700',
  //       day_outside:
  ...
  //         if (props.name === 'years') {
  //           const { fromDate, fromMonth, fromYear, toDate, toMonth, year, } = useDayPicker();
  ...
  //               <button className="flex items-center gap-1 p-0 m-0" onClick={() => setShowYear(!showYear)}>
  //                 {props.caption}
  //                 <ChevronDown className="h-4 w-4 " />
  //               </button>
  //               {showYear && (
  //                 <div>
  //                   <div className={cn(['absolute bg-neutral-white z-50 inset-0 py-5  rounded-lg'])}>
  //                     <ScrollArea className="h-full w-full" type="hover">
  //                       <div className="grid grid-cols-3 gap-4 px-8 ">
  //                         {selectYears.map((selectYears) => (
  //                           <Button
  //                             variant={activeIndexYear === selectYears.value ? 'primary' : 'ghost'}
  //                             key={selectYears.value}
  //                             onClick={() => {
  //                               newDate.setFullYear(parseInt(selectYears.value));
  //                               setActiveIndexYear(selectYears.value);
  //                             }}
  //                             className={cn(activeIndexYear !== selectYears.value && 'text-content-body')}
  //                           >
  //                             <span className="text-b1">{selectYears.label}</span>
  //                           </Button>
  //                         ))}
  //                       </div>
  //                     </ScrollArea>
  ...
  //                         onClick={() => {
  //                           newDate.setFullYear(parseInt(props.value as string) - 1);
  //                           goToDate(newDate);
  //                         }}
  //                       >
  //                         <ChevronLeft className="h-6 w-6" />
  //                       </Button>
  //                       <span className="text-b1 align-middle h-fit">{props.caption}</span>
  //                       <Button
  //                         variant="ghost"
  //                         className="bg-transparent p-0 opacity-50 hover:opacity-100"
  //                         onClick={() => {
  ...
  //                           <Button
  //                             key={i}
  //                             variant={activeIndexMonth === items.value ? 'primary' : 'ghost'}
  //                             onClick={() => {
  //                               newDate.setMonth(parseInt(items.value));
  //                               setActiveIndexMonth(items.value);
  //                             }}
  //                             className={cn(activeIndexMonth !== items.value && 'text-content-body')}
  //                           >
