      ...
      cell: function Cell({ row }) {
        ...
        return (
          <TableRowActionsMenu
            canUpdate={canUpdate}
            canDelete={canDelete}
            onEdit={() => router.push(`/dashboard/tribunal-review/${row.original.id}/edit`)}
            onView={() => router.push(`/dashboard/tribunal-review/${row.original.id}/view`)}
            onSendEmail={handleSendEmail}
            recipientEmail={row.original.email}
            deleteTitle="Delete this tribunal review"
            deleteDescription="Are you sure you want to delete this tribunal review? Deleting this tribunal review will remove all associated data, including contacts, interactions and notes."
            deleteLabel="Delete tribunal review"
