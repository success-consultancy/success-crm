      ...
      cell: function Cell({ row }) {
        ...
        return (
          <TableRowActionsMenu
            canUpdate={canUpdate}
            canDelete={canDelete}
            onEdit={() => router.push(`/dashboard/visa/${row.original.id}/edit`)}
            onView={() => router.push(`/dashboard/visa/${row.original.id}/view`)}
            onSendEmail={handleSendEmail}
            recipientEmail={row.original.email}
            deleteTitle="Delete this visa"
            deleteDescription="Are you sure you want to delete this visa? Deleting this visa will remove all associated data, including contacts, interactions and notes."
            deleteLabel="Delete visa"
